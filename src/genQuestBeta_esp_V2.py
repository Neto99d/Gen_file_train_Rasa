...
# ............DEVUELVE PREGUNTAS mediante spacy Entidades y DEVUELVE LAS RESPUESTAS POR MODELO Entrenado.......
...
def genQuest(asunto, user):
    ...
    for sent in sentences:
        ...
        for ent in docSent.ents:
            ...
            '''if(organizacion != "" and personaje != ""):
                questions.append('Queien es {} para {}'.format(
    ...
    top_k = 1
    # paso la lista de oraciones y obtengo los datos del modelo entrenado al analizarlas
    corpus_embeddings = model.encode(sentencesSTR, convert_to_tensor=True)
    for pregunta in questionsFIX:
        print()
        # paso la pregunta al modelo
        embedding1 = model.encode(pregunta, convert_to_tensor=True)

        # calcula la similaridad entre las oraciones y las preguntas
        cos_scores = util.pytorch_cos_sim(
            embedding1, corpus_embeddings)[0]

# Ordene los resultados en orden decreciente y obtenga el primer top_k
        top_results = np.argpartition(-cos_scores, range(top_k))[0:top_k]

        print("Oración:", pregunta, "\n")
        print("Top", top_k, "Más similar de la lista:")
        for idx in top_results[0:top_k]:
            print(sentencesSTR[idx], "(Puntaje: %.4f)" % (cos_scores[idx]))
            result = qa_pipeline({
